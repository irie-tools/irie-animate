...
import { importStorefront } from "@/src/lib/storefrontIntake";
...
export async function POST(request: Request) {
  try {
    ...
    if (!url.trim()) return NextResponse.json({ ok: false, error: "Enter a storefront URL." }, { status: 400 });
    const intake = await importStorefront(url);
    ...
    return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : "Storefront import failed." }, { status: 500 });
  }
}
